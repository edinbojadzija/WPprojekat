...
void Snake::Update(HWND hWnd)
{
    ...
    else if (Snake::GetCellType(head.row, head.col) == SNAKE)
    {
        ...
        MessageBox(hWnd, TEXT(buff), "Game Over!", MB_OK | MB_ICONWARNING);
        DestroyWindow(hWnd);
    }
...
Snake::CellType Snake::GetCellType(int row, int col)
{
    ...
    return EMPTY; // ne bi trebao nikada
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        if(!SetTimer(hWnd, ID_TIMER, 55, NULL))
        {
            MessageBox(hWnd, "Couldn't set timer!", "Error", MB_OK | MB_ICONEXCLAMATION);
            PostQuitMessage(1);
        }
